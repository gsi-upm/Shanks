...
public abstract class ProbabilisticNetworkElementEvent extends ProbabilisticEvent{
    ...
    private Logger logger = Logger.getLogger(ProbabilisticNetworkElementEvent.class.getName());
    ...
    public ProbabilisticNetworkElementEvent(String name, Steppable generator, Double prob) {
...
//    NOTE: this method will have sense when properties are implemented for scenarios.  
//    public void addPossibleAffectedScenarioProperty(Class<? extends Scenario> scenarioClass, String property, Object value){
//        this.addPossibleAffectedScenarioField(scenarioClass, property, value);
//    }
    
    public void addPossibleAffectedScenarioField(Class<? extends Scenario> scenarioClass, String field, Object value){
        if(this.possibleAffectedScenarios.containsKey(scenarioClass)){
            HashMap<String, Object> affectedFields = this.possibleAffectedScenarios.get(scenarioClass);
            if(!affectedFields.containsKey(field)){
                ...
                this.logger.warning("Trying to add a new affected field that already has beeen defined. May be you has define " +
                                    ...
                                    "was ignored.\nScenarioClass: "+scenarioClass.getName()+"\nField: "+field+"\nValue: "+value);
            }
        } else {
    ...
    public void addPossibleAffectedElementField(Class<? extends NetworkElement> elementClass, String field, Object value) {
        if(this.possibleAffectedElements.containsKey(elementClass)){
            HashMap<String, Object> affectedFields = this.possibleAffectedElements.get(elementClass);
            if(!affectedFields.containsKey(field)){
                affectedFields.put(field, value);
            } else {
                this.logger.warning("Trying to add a new affected field that already has beeen defined. May be you has define " +
    ...
    public boolean removePossibleAffectedElements(Class<? extends NetworkElement> elementClass) {
        if (this.possibleAffectedElements.containsKey(elementClass)) {
    ...
    public boolean removePossibleAffectedScenarios(final Class<? extends Scenario> scenarioClass) {
        if (this.possibleAffectedScenarios.containsKey(scenarioClass)) {
    ...
    public void changeProperties() throws UnsupportedNetworkElementFieldException {
        List<? extends NetworkElement> elements = new ArrayList<NetworkElement>(this.affectedElements);
                ...
                if(c.equals(el.getClass())){                        
                    for(String s : possibleAffectedElements.get(c).keySet()){
                        if(el.getProperties().containsKey(s)){
                            el.updatePropertyTo(s, possibleAffectedElements.get(c).get(s));
                            elements.remove(el);
                        }
                    }
                }
                ...
                if(c.equals(el.getClass())){                        
                    for(String s : possibleAffectedElements.get(c).keySet()){
                        if(el.getStatus().containsKey(s)){
                            el.updateStatusTo(s, (Boolean) possibleAffectedElements.get(c).get(s));
                            elements.remove(el);
                        }
                    }
                }
