package es.upm.dit.gsi.shanks.tutorial.model.adsl.element.device;
...
public class DSLAM extends Device {
    ...
    public static final String PROPERTY_TEMPERATURE = "Temperature"; // 0ºC-100ºC
    ...
    public void checkStatus() throws UnsupportedNetworkElementStatusException {
        ...
        if ((temp == null)||(cong==null)) {
        	this.updateStatusTo(DSLAM.STATUS_UNKOWN);
        } else {
        	if ((temp>50)&&(cong>0.3))
                this.updateStatusTo(DSLAM.STATUS_NOK);
        	else if (temp>70) 
            	this.updateStatusTo(DSLAM.STATUS_HIGHTEMP);
        	else if (cong>0.5) 
            	this.updateStatusTo(DSLAM.STATUS_CONGESTED);
        	else 
        		this.updateStatusTo(DSLAM.STATUS_OK);
        }
    }
}