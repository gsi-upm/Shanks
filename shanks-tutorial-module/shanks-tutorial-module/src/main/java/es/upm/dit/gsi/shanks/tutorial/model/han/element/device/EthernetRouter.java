...
public class EthernetRouter extends Device {
	...
	public void checkProperties()
		...
		if (status.equals(EthernetRouter.STATUS_OFF)) {
			this.shutdown();
	...
	public void checkStatus() throws UnsupportedNetworkElementStatusException {
		...
		if (power.equals("OFF")) {
			...
			this.shutdown();
				...
				this.updateStatusTo(EthernetRouter.STATUS_DISCONNECTED);
		} else if (connection.equals("NO-IP") ){
			this.updateStatusTo(EthernetRouter.STATUS_NOISP_SERVICE);
		} else if (congestion>30){
			this.updateStatusTo(EthernetRouter.STATUS_CONGESTED);
		} else {
