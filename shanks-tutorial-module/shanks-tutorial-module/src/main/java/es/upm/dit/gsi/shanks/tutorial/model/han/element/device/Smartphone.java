...
public class Smartphone extends Device{
	...
	public void checkStatus() throws UnsupportedNetworkElementStatusException {
		...
		if (power.equals("OFF")) {
			this.updateStatusTo(Smartphone.STATUS_OFF);
			this.shutdown();
		} else {
			if (battery <= 5) {
				this.updateStatusTo(Smartphone.STATUS_DISCHARGED);
			} else if (signal > 50) {
				if (connection.equals("Connected")) {
					this.updateStatusTo(Smartphone.STATUS_OK);
				} else {
					this.updateStatusTo(Smartphone.STATUS_DISCONNECTED);
				}
			} else {
				this.updateStatusTo(Smartphone.STATUS_OUT_OF_RANGE);
			}
		}
	}
